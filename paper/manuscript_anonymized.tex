...
% Translation
% ============================================================
% Target: Knowledge-Based Systems (KBS)
...
% Simple Arabic text command — just switches font, XeTeX handles RTL automatically
...
A Tantivy inverted index (255\,MB) is built over the 3.4 million TOC entries to enable efficient full-text search. The corpus spans all four Sunni legal schools (Hanafi, Maliki, Shafi'i, Hanbali) plus cross-\textit{madhhab} comparative works, and covers jurisprudence, Quranic exegesis, hadith collections, theology, Sufism, and Arabic grammar.
...
Transliterated Arabic terms common in Indonesian discourse (e.g., \textit{shalat}~$\rightarrow$~\ar{صلاة}, \textit{wudhu}~$\rightarrow$~\ar{وضوء}, \textit{riba}~$\rightarrow$~\ar{ربا}) are mapped to their Arabic originals using a curated dictionary of 200+ entries covering the domains of worship (\textit{ibadah}), commercial law (\textit{muamalat}), family law (\textit{munakahat}), and creed (\textit{aqidah}).

\subsubsection{Stage 3: Phrase Mapping}
Multi-word phrases are detected and mapped to corresponding Arabic compound expressions. For example, ``nikah siri'' (unregistered marriage) maps to \ar{نكاح بغير ولي} and \ar{نكاح سري}, while ``shalat jamak'' (combined prayers) maps to \ar{الجمع بين الصلاتين}. This stage captures semantic units that single-term translation would miss.
...
\begin{enumerate}
    ...
    \item \textbf{Concept conflation}: Arabic's triliteral root system means that morphological variants can span semantically distinct concepts (e.g., \ar{صلاة}~[prayer] and \ar{صلة}~[connection] share the root \ar{ص}-\ar{ل}-\ar{و}/\ar{ي}).
\end{enumerate}
...
\begin{itemize}
    \item \ar{صلاة} (prayer) and \ar{صلوات} (prayers/blessings) are stemmed to the same root but appear in different juristic contexts.
    \item \ar{نكاح} (marriage contract) and \ar{منكوحة} (married woman) have different retrieval profiles in family law chapters.
    \item \ar{حكم} (ruling) and \ar{حاكم} (judge) and \ar{محكمة} (court) share a root but denote distinct concepts.
\end{itemize}
...
During the preparation of this work the author used GitHub Copilot (Claude Opus 4.6) in order to assist with code implementation for the evaluation framework, generation of data extraction scripts, and LaTeX document formatting. After using this tool, the author reviewed and edited the content as needed and takes full responsibility for the content of the published article. All scientific analysis, interpretation of results, domain expertise contributions, and intellectual content are solely the work of the author. The system design, query translation dictionaries, gold standard construction, and all research conclusions were developed independently by the author.
...
The gold standard evaluation dataset, query translation dictionaries, and evaluation scripts are available at the project repository. The classical Arabic text corpus is sourced from publicly available digital libraries of Islamic heritage.
...
\midrule
\textit{Ibadah} & shalat, prayer & \ar{صلاة، الصلاة، الصلوات} \\
& wudhu, ablution & \ar{وضوء، الوضوء، الطهارة} \\
& puasa, fasting & \ar{صوم، صيام، الصيام} \\
& zakat, zakah & \ar{زكاة، الزكاة، زكاة المال} \\
& haji, hajj & \ar{حج، الحج، المناسك} \\
\midrule
\textit{Muamalat} & riba, interest & \ar{ربا، الربا، فائدة، ربا الفضل، ربا النسيئة} \\
& jual beli, trade & \ar{بيع، المعاملات} \\
& gadai, collateral & \ar{رهن} \\
\midrule
\textit{Munakahat} & nikah, marriage & \ar{نكاح، زواج، التزويج، عقد النكاح} \\
& cerai, divorce & \ar{طلاق، الطلاق، فراق} \\
& mahar, dowry & \ar{مهر، صداق} \\
\midrule
\textit{Aqidah} & tauhid, monotheism & \ar{توحيد} \\
& bid'ah, innovation & \ar{بدعة} \\
& murtad, apostasy & \ar{ردة} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
id & ibadah & hukum shalat sambil menggendong anak kecil & \ar{حمل الطفل في الصلاة، حمل النجاسة} \\
id & muamalat & boleh gak gadai emas untuk modal usaha & \ar{رهن الذهب، الرهن في المعاملة} \\
en & munakahat & wife requesting divorce due to husband impotence & \ar{خلع، فسخ النكاح بالعيوب، العنة} \\
mixed & ibadah & shalat jamak qasar saat traveling by plane & \ar{الجمع والقصر، السفر، صلاة المسافر} \\
ar & aqidah & \ar{حكم التوسل بالأنبياء والصالحين} & \ar{التوسل، الاستغاثة، الشفاعة} \\
\bottomrule
\end{tabular}
\end{table*}
